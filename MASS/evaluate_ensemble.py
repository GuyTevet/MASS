...
def get_parser():
    ...
    parser.add_argument("--data_path", type=str, default="",
                        help="Data path")
    parser.add_argument("--test", action='store_true', help='Use test set. Otherwise use validation set.')
...
def evaluate_models(decoders, encodeds, params):
    pass
...
def main(params):
    ...
    # context_sent = ['Poly@@ gam@@ ie statt Demokratie .']
    # context_sent += ['Trump was born and raised in the New York City borough of Queens, and received an economics degree from the Wharton School. He took charge of his familys real estate business in 1971, renamed it The Trump Organization, and expanded it from Queens and Brooklyn into Manhattan.']

    _set = 'test' if params.test else 'valid'
    _type = params.src_lang + '-' + params.tgt_lang
    context_path, resp_path = [os.path.join(params.data_path, '.'.join([_set, _type, s])) for s in _type.split('-')]


    context_sent = [] # input context
    with open(context_path, 'r') as context_f:
        for line in context_f.readlines():
            assert len(line.strip().split()) > 0
            context_sent +=[line] # * params.samples_per_source

    resp_sent = [] # input context
    with open(resp_path, 'r') as resp_f:
        for line in resp_f.readlines():
            assert len(line.strip().split()) > 0
            resp_sent +=[line] # * params.samples_per_source

    f = io.open(params.output_path, 'w', encoding='utf-8')

    for i in range(0, len(context_sent), params.batch_size):

        # prepare batch
        word_ids = [torch.LongTensor([dico.index(w) for w in s.strip().split()])
                    for s in context_sent[i:i + params.batch_size]]
        ...
        evaluate_models(decoders, encodeds, params)
